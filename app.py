...
def _log_response(response):
    if hasattr(g, "start_time"):
        ...
        logger.info("[%s] → %d (%.1f ms)", getattr(g, "request_id", "?"), response.status_code, ms)
...
def err(message: str, status: int = 400):
    ...
    logger.warning("[%s] Error %d: %s", req_id, status, message)
    return jsonify({"error": message, "request_id": req_id, "status": status}), status
